...
const service = ({ strapi }: { strapi: Core.Strapi }) => ({
  ...
  async canApprove(reviewId: string, userId: number): Promise<boolean> {
    const reason = await this.getApprovalBlockReason(reviewId, userId);
    return reason === null;
  },

  async getApprovalBlockReason(
    reviewId: string,
    userId: number,
  ): Promise<ApprovalBlockReason | null> {
  ...
  getApprovalBlockMessage(reason: ApprovalBlockReason): string {
